...
@torch.no_grad()
def _run_frame_cached(human_gs, canon_out, scene_out_cached, data, bg_color, render_mode,
                      t_deform, t_render, t_pipeline):
    """Canonical-cached inference: triplane + MLP decoders (canon_forward)
    and scene_gs.forward() are computed ONCE outside the loop. Per frame we
    only redo SMPL + LBS deformation (forward_test) and the rasterizer.
    This is the "rendering speed" most papers report for animated human
    methods — the canonical gaussians don't change between frames, only
    the pose does."""
    with CudaTimer(t_pipeline):
        if human_gs is not None:
            with CudaTimer(t_deform):
                human_out = human_gs.forward_test(
                    canon_forward_out=canon_out,
                    global_orient=data['global_orient'],
                    body_pose=data['body_pose'],
                    betas=data['betas'],
                    transl=data['transl'],
...
def benchmark(cfg):
    ...
    # ----- Canonical-cached inference (the "rendering speed" from the paper) -----
    # canon_forward() runs triplane + appearance/geometry/deformation decoders
    # once; its outputs only depend on get_xyz which is static at inference.
    # scene_gs.forward() is also pose-independent, so we cache it too.
    # Per frame we only redo SMPL/LBS deformation + rasterization.
    t_cached_pipeline, t_cached_deform, t_cached_render = [], [], []
    cached_enabled = human_gs is not None and hasattr(human_gs, 'canon_forward')
    if cached_enabled:
        canon_out = human_gs.canon_forward()
        scene_out_cached = scene_gs.forward() if scene_gs is not None else None

        for i in range(WARMUP_ITERS):
            data = dataset[i % len(dataset)]
            _run_frame_cached(human_gs, canon_out, scene_out_cached, data,
                              bg_color, render_mode, [], [], [])
        torch.cuda.synchronize()

        for data in tqdm(dataset, desc='FPS benchmark (cached)'):
            _run_frame_cached(human_gs, canon_out, scene_out_cached, data,
                              bg_color, render_mode,
                              t_cached_deform, t_cached_render, t_cached_pipeline)
    ...
    return {
        ...
        'pipeline_cached': _stats(t_cached_pipeline),
        'cached_stages': {
            'smpl_lbs_deform': _stats(t_cached_deform),
            'rasterization': _stats(t_cached_render),
        },
    }
...
def _print_report(results):
    ...
    print('-' * 70)
    print(' Full pipeline (end-to-end per frame, triplane+MLPs every frame):')
    _print_row('pipeline_full', results['pipeline_full'])
    print('-' * 70)
    print(' Stage breakdown (full pipeline):')
    for name, st in results['stages'].items():
        if st['count'] == 0:
            continue
        _print_row(name, st)
    if results.get('pipeline_cached', {}).get('count', 0) > 0:
        print('-' * 70)
        print(' Canonical-cached inference (triplane+MLPs run once, reused):')
        _print_row('pipeline_cached', results['pipeline_cached'])
        print(' Stage breakdown (cached):')
        for name, st in results['cached_stages'].items():
            if st['count'] == 0:
                continue
            _print_row(name, st)
    print('=' * 70)
    print(' Notes:')
    print('  - pipeline_full re-runs triplane + MLP decoders EVERY frame.')
    print('  - pipeline_cached runs canon_forward() and scene_gs.forward()')
    print('    ONCE up front (the canonical gaussians do not depend on pose)')
    print('    and only re-runs SMPL+LBS deformation + rasterization per')
    print('    frame. This matches how animation / novel-pose rendering')
    print('    is actually deployed, and is the "rendering speed" typically')
    print('    reported in the paper.')
    print('  - triplane_only / mlp_decoders_only are isolated micro-benchmarks')
    print('    (subsets of human_forward_total).')
    print()
