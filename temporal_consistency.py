...
def masked_l1(a: np.ndarray, b: np.ndarray, mask: np.ndarray = None) -> float:
    ...
    diff = diff.mean(axis=-1) if diff.ndim == 3 else diff  # HxW
    denom = mask.sum() + 1e-8
    return float((diff * mask).sum() / denom)


# ─────────────────────────────────────────────────────────────────────────────
# Model construction (mirrors the validation pipeline)
# ─────────────────────────────────────────────────────────────────────────────
def build_model(cfg, ckpt_path):
    # Import datasets lazily. Some environments don't have all optional deps
    # installed (e.g. ZJU vs NeuMan), but temporal consistency should still run
    # for the dataset specified by the config.
    ...
    if cfg.dataset.name == "zju":
        ...
        dataset = NeumanDataset(
            cfg.dataset.seq,
            "val",
            render_mode=getattr(cfg, "mode", "human_scene"),
        )
...
def run(cfg_path, ckpt_path, n_pairs=50, output_json=None, use_lpips=True):
    ...
    # Perceptual model for tLP (net='alex' matches the per-frame LPIPS used elsewhere)
    lpips_model = None
    if use_lpips:
        try:
    ...
    # Some Neuman val splits contain only a handful of frames; fall back to train.
    requested_pairs = n_pairs
    if len(dataset) < 3:
        logger.warning(
            f"Val split has only {len(dataset)} frames -> too few for temporal metrics. "
    ...
    if n_pairs < 2:
        raise RuntimeError(f"Not enough frames for temporal metrics (got {len(dataset)}).")
    logger.info(f"Rendering {n_frames} consecutive frames (n_pairs={n_pairs}) ...")

    rendered_np = []  # list of HxWx3 uint8
    rendered_tensor = []  # list of 3xHxW cuda float for LPIPS
    gt_frames = []
    masks = []
    splat_means = []  # list of (N,3) for per-splat accel
    ...
    for idx in range(n_frames):
        ...
        # Different branches/models return different structures:
        # - Cloth-aware wrapper: {"body": {...}, "cloth": {...}}
        # - Basic HUGS: {...} directly (no "body" key)
        if isinstance(human_pack, dict):
            if "body" in human_pack:
        ...
        img_tensor = render_pkg["render"].clamp(0, 1)  # 3xHxW
    ...
    for i in range(n_pairs):
        ...
        occ = occlusion_mask(gt_t, gt_t1, flow, alpha=50.0)
        ...
        gt_warp_baseline.append(
            masked_l1(
                warp_image(gt_t.astype(np.float32) / 255.0, flow),
                gt_t1.astype(np.float32) / 255.0,
            )
        )

    residual_stack = np.stack(flicker_residuals, axis=0)  # TxHxWx3
    ...
    if len(splat_means) >= 3:
        ...
        if len(set(sizes)) == 1:
            X = torch.stack(splat_means, dim=0)  # TxNx3
            accel = X[2:] - 2 * X[1:-1] + X[:-2]  # (T-2)xNx3
            accel_norm = accel.norm(dim=-1)  # (T-2)xN
    ...
    print(f"  Flicker (resid sigma) : {flicker_mag:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_pairs", type=int, default=50, help="Number of consecutive frame pairs to evaluate"
    )
    parser.add_argument("--output_json", default=None)
    parser.add_argument("--no_lpips", action="store_true", help="Disable tLP (skip LPIPS model load)")
    args = parser.parse_args()

    run(args.cfg, args.ckpt, args.n_pairs, args.output_json, use_lpips=not args.no_lpips)
